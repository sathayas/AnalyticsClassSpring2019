...
# loading the file with IMDB reviews
f = open('SentimentReviews/imdb_labelled.txt','r')
reviewList = []
while True:
    lineData = f.readline().strip()
    if lineData:
...
# extracting features for the IMDB reviews
imdbSet = [(document_features(d), c) for (d,c) in reviewList]

# classification accuracy
print('Classifier performance on IMDB reviews:')
print(nltk.classify.accuracy(clf, imdbSet))




# loading the file with yelp reviews
f = open('SentimentReviews/yelp_labelled.txt','r')
reviewList = []
while True:
    lineData = f.readline().strip()
    if lineData:
...
# extracting features for the yelp reviews
yelpSet = [(document_features(d), c) for (d,c) in reviewList]

# classification accuracy
print('Classifier performance on Yelp reviews:')
print(nltk.classify.accuracy(clf, yelpSet))
